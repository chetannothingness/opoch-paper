...
% -------------------------------------------------------------------------
\subsection{Positive Evaluation and Manifestation Readout}
\label{sec:context-born:manifestation}

The Born rule is not only quantum probability.  It is also the
operator-algebraic face of local readout and manifestation.  Positive
evaluation on the C$^*$-algebra selects which distinctions are manifested
at a given support---exactly the same operation that the manifestation
current $J_q = \Lambda_{M_t}(b_q)$ performs in the operational layer
(\cref{sec:manifestability}).  Quantum probability and local manifestation
are two views of one structure: positive evaluation on the witness algebra.
