% =============================================================================
% Section: Final Compression and Local Manifestation
% =============================================================================
...
% -------------------------------------------------------------------------
\subsection{Quantitative and Operational Extraction Frontiers}
\label{sec:discussion:frontiers}

The structure is complete at three levels: structural (A0$^*$ $\to$ K\"{a}hler),
quantitative (seed $\to$ spectrum), and operational ($\chi(W)$ $\to$
value equation $\to$ residual kernel).  Every dimensionless invariant is
forced (Theorem~\ref{thm:observable-decomp}); every dimensional quantity is
fixed up to the internally derived normalization map.  The remaining work
is explicit spectral extraction from the seed operator and deployment of
the residual kernel compiler to specific problem families.
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \textbf{Residual kernel deployment.}
        Deploying the residual kernel compiler to specific problem families
        (SAT, scheduling, MAPF) and measuring the polynomial bound in
        practice.  The theoretical bound is (fullSize+1)$^4$; practical
        bounds may be tighter.

  \item \textbf{Law mining.}
        The law-mining block (\texttt{LawMining/}) enables Lean to verify
        candidate residual signatures for completeness, minimality, and
        polynomial bound.  Extending this to automatically discover
        efficient problem encodings.

  \item \textbf{Lean mechanization of infinite-domain theorems.}
        The 140-file Lean~4 verification covers the finite/Nat model.
