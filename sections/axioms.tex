...
\medskip\noindent
\emph{Lean formalization note.}
In the companion Lean~4 development, the forward direction of
A0$^*$ ($\mathit{IsReal}\;\delta \Rightarrow \exists\, w,\;\ldots$) is
derived in \texttt{EndogenousMeaning.lean} from the
\texttt{Nothingness} structure via theorems N1--N5.  The full
biconditional is packaged as the sole \texttt{axiom} declaration in
\texttt{Axioms.lean} for downstream use; the backward direction
encodes the semantic closure that ``real'' means ``witnessed.''
When this paper states ``no declared axioms,'' it refers to the
paper's logical structure; when the artifact description states
``one axiom,'' it refers to the Lean packaging of the derived
biconditional.
\end{remark}
...
% -------------------------------------------------------------------------
\subsection{Questions as Derived Admissible Selector-Forms}
\label{sec:axioms:questions}

Admissible questions are not external semantic objects imported from
outside the framework.  They are derived finite selector-forms over the
same witness carrier already forced by A0$^*$.

A question $q$ is a finite specification of which truth-quotient classes
to distinguish.  It is itself an element of $\FinSlice$---a finite binary
string encoding a partial selector over $\TQ{\Ledger}$.  The distinction
``$q$ is a real question'' is witnessable by the same finite procedures
that A0$^*$ demands for any other distinction.

Questions inherit reality status only insofar as they are themselves
witness-stable coordinate forms: a question that cannot be stably posed
under all admissible witnesses is not an admissible question.  This is
not a new doctrine; it is A0$^*$ applied to the act of questioning.
