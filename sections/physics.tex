...
% -------------------------------------------------------------------------
\subsection{Physics as Readout of Latent Indistinguishability-Energy}
\label{sec:physics:readout}

All physical structures derived above are readouts of one deeper object:
the latent indistinguishability-energy $\mathcal{U}_{\mathrm{ind}}$ on
the refinement atlas $X$ (\cref{sec:manifestability}).

\begin{itemize}[nosep]
  \item \textbf{Gravity:} symmetric curvature/accessibility readout of
        $\mathcal{U}_{\mathrm{ind}}$.
  \item \textbf{Quantum theory:} antisymmetric/current/phase readout
        ($J_x = \Omega^{-1}\eta_x$).
  \item \textbf{Gauge fields:} redundancy/invariance in the refinement atlas.
  \item \textbf{Mass:} latent-energy gap ($\chi(W)$ of first accessible
        distinction).
  \item \textbf{Thermodynamics:} irreversible witness/ledger aspect of
        $\mathcal{U}_{\mathrm{ind}}$.
  \item \textbf{Spacetime:} a geometric chart on the refinement atlas, not
