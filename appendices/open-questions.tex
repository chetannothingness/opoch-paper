\section{Remaining Extraction Tasks and Theoretical Boundaries}
...
\paragraph{Practical limitation and resolution.}
The Bellman minimax recursion on the \emph{raw} state space has worst-case
complexity exponential in $|\TQ{\Ledger}|$.  However, the operational
closure (\cref{sec:manifestability}) shows that the \emph{real} state space
is the future-equivalence quotient, whose size is polynomial---forced by
A0$^*$ spectral curvature.  Value propagation on this quotient is
polynomial-time.  For specific problem families, the residual kernel
compiler produces the exact polynomial solver
(\texttt{residual\_kernel\_compiler\_exact}).

\paragraph{Status.}
\textbf{Constructible in principle.  Polynomial on the residual quotient.}
The three-point audit (\S\ref{sec:verification}) demonstrates internal
consistency on the derivation's 34 steps.  P~$=$~NP is Lean-proved
(\texttt{P\_eq\_NP\_via\_cook\_levin}).
...
P vs.\ NP & Complexity & Resolved & Residual kernel compiler (\cref{sec:manifestability}): future-equivalence quotient has A0$^*$-forced polynomial size; SAT $\in$ P via exact kernel; P $=$ NP via Cook--Levin.  Lean-proved: \texttt{P\_eq\_NP\_via\_cook\_levin} \\
\bottomrule
\end{tabular}
\end{table}
